...
if (require.main === module) {
	...
	(async () => {
		...
		try {
			...
			const { pipeline_connection, logDir, cacheDir, sessionId } = await require("../common/setup")(argv.env, argv.resumeSessionId);
				...
				try {
					...
					if (fs.existsSync(textFileName)) {
						...
						try {
							...
							const [casePDFResult] = await pipeline_connection.query(
								"INSERT INTO case_pdfs SET ?",
								{
							...
							const [caseResult] = await pipeline_connection.query(
			...
			const [pdfsThatExist] = await pipeline_connection.query(
				"SELECT pdf_db_key, pdf_id FROM case_pdfs"
			);
				...
				find('name', 'WINWORD.EXE').then(function (list) {
					...
					if(wordProcessesCount !== lastWordProcessesCount) {
						...
						if(processedWordFileSizes.length > 0) {
							console.log(`- Average Word file size: ${Math.round((processedWordFileSizes.reduce((accumulator, current) => accumulator + current, 0) / processedWordFileSizes.length))}kb`);
						}
					...
					if(hasInitialised) {
						...
						if(wordProcessesCount <= adapterConfig.start_pdf_processor_at_word_processors_count && !pdfWorkerCanStop) {
