...
const run = async (connection, logDir) => {
	...
	const [ results ] = await connection.task((t) => {
		return t.batch([
			t.query(`select id, case_text from cases;`),
			t.query(`select citation, case_id from case_citations;`)
		]);
	});
	...
	for (var key in case_citations) {
		...
		for (var count_key in mapped_count) {
			insertQueries.push(
				`replace into cases_cited (case_origin, case_cited, citation_count) values ('${key}', '${count_key}', ' ${mapped_count[
					count_key
				]}');`
			);
		}
